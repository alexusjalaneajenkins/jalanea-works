...
import {
    ...
    Camera, BookOpen, Video, Car, Bus, Bike, Footprints, Baby, Clock, Briefcase, DollarSign, Mail
} from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode, onComplete, setRoute }) => {
    ...
    const [signupError, setSignupError] = useState<{ message: string; isEmailExists: boolean } | null>(null);
    ...
    const handleEmailAuth = async () => {
        ...
        // For login, authenticate and complete
        ...
        setIsSaving(true);
        try {
            await signup(signupEmail, signupPassword, fullName || signupEmail.split('@')[0]);
            onComplete(fullName || signupEmail.split('@')[0], '');
        } catch (error: any) {
            console.error("Signup failed", error);
            const errorCode = error.code || '';
            if (errorCode === 'auth/email-already-in-use' || error.message?.includes('email-already-in-use')) {
                setSignupError({ message: 'An account with this email already exists.', isEmailExists: true });
            } else {
                const errorMsg = error.message?.replace('Firebase: ', '').replace(/\(auth\/[^)]+\)/, '').trim() || 'Failed to create account';
                setSignupError({ message: errorMsg, isEmailExists: false });
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const handleForgotPassword = async () => {
        if (!signupEmail) {
            setSignupError({ message: 'Please enter your email address first', isEmailExists: false });
            return;
        }
        try {
            const { sendPasswordResetEmail } = await import('firebase/auth');
            await sendPasswordResetEmail(auth, signupEmail);
            setSignupError({ message: `Password reset email sent to ${signupEmail}. Check your inbox!`, isEmailExists: false });
        } catch (error: any) {
            console.error('Password reset failed:', error);
            setSignupError({ message: 'Failed to send reset email. Please try again.', isEmailExists: false });
        }
    };
    ...
    const renderSignup = () => (
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300 max-w-sm mx-auto w-full py-2">
            {/* Branded Header */}
            <div className="text-center space-y-4">
                <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-gold/30 to-gold/10 border border-gold/40 shadow-lg shadow-gold/20 mb-2">
                    <Zap size={28} className="text-gold" fill="currentColor" />
                </div>
                <div>
                    <h2 className="text-2xl font-display font-bold text-white">Join JalaneaWorks</h2>
                    <p className="text-jalanea-300 text-sm mt-2">
                        Get matched with jobs that actually want <span className="text-gold font-medium">your degree</span>.
                    </p>
                </div>
            </div>

            {/* Google Sign Up - Premium Button */}
            <button
                onClick={handleGoogleAuth}
                className="w-full flex items-center justify-center gap-3 bg-gold hover:bg-gold-light text-jalanea-950 font-bold py-3.5 px-4 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl hover:shadow-gold/30 hover:-translate-y-0.5 group"
            >
                {/* Google "G" Icon */}
                ...
                <ArrowRight size={16} className="opacity-0 -ml-5 group-hover:opacity-100 group-hover:ml-0 transition-all" />
            </button>

            {/* Trust Badge */}
            <div className="flex items-center justify-center gap-2 text-xs text-jalanea-400">
                <CheckCircle2 size={14} className="text-green-400" />
                <span>2-minute setup • No credit card required</span>
            </div>

            {/* OR Divider */}
            <div className="relative flex items-center py-1">
                <div className="flex-grow border-t border-white/10"></div>
                <div className="mx-4 flex items-center gap-2 text-xs font-medium text-jalanea-500 uppercase tracking-wider">
                    <span className="inline-block w-1.5 h-1.5 rounded-full bg-jalanea-600"></span>
